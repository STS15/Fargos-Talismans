...
public class AncientHornItem extends TalismanItem implements ICurioItem, Ancient_Horn_Provider {

    private static final Logger LOGGER = LogManager.getLogger(Fargos.MODID);
    public static final String charmName = "ancient_horn";
    ...
    public static class Events {
        ...
        public static void onFall(LivingFallEvent event) {
            ...
            if (event.getDistance() <= 2.0F) return;

            boolean equipped = CuriosApi.getCuriosHelper().findEquippedCurio(
                    stack -> stack.getItem() instanceof Ancient_Horn_Provider, player).isPresent();

            boolean enabled = TalismanUtil.isTalismanEnabled(player, charmName);
            if (!equipped || !enabled) return;

            player.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SPEED, 30, 1, true, true));
        }
    }
}
