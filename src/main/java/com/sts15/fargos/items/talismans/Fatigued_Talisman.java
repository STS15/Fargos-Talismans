...
public class Fatigued_Talisman extends TalismanItem implements Fatigued_Talisman_Provider {
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltipComponents, TooltipFlag tooltipFlag) {
        double configValue = Config.FATIGUED_TALISMAN_INCREASED_DAMAGE.getAsDouble() * 100;
        ...
        public static void onLivingDamage(LivingIncomingDamageEvent event) {
            ...
            if (source instanceof ServerPlayer player) {
                if (player.hasEffect(MobEffects.HUNGER)) {
                    if (player.hasEffect(EffectsInit.FATIGUED_TALISMAN_EFFECT) || CuriosApi.getCuriosHelper().findEquippedCurio(stack -> stack.getItem() instanceof Fatigued_Talisman_Provider, player).isPresent()) {
                        if (!TalismanUtil.isTalismanEnabled(player, talismanName))
                            return;
                        event.setAmount(event.getAmount() * (1f + Config.FATIGUED_TALISMAN_INCREASED_DAMAGE.get().floatValue()));
                    }
                }
            }
        }
