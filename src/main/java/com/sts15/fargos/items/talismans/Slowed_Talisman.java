...
public class Slowed_Talisman extends TalismanItem implements Slowed_Talisman_Provider {

    private static final String talismanName = "slowed_talisman";

    public Slowed_Talisman() {
        super(new Properties().rarity(Rarity.UNCOMMON));
    }

    @Override
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltipComponents, TooltipFlag tooltipFlag) {
        double configValue = Config.SLOWED_TALISMAN_INCREASED_DAMAGE.getAsDouble() * 100;
                ...
                if (player.hasEffect(MobEffects.MOVEMENT_SLOWDOWN)) {
                    if (player.hasEffect(EffectsInit.SLOWED_TALISMAN_EFFECT) || CuriosApi.getCuriosHelper().findEquippedCurio(stack -> stack.getItem() instanceof Slowed_Talisman_Provider, player).isPresent()) {
                        if (!TalismanUtil.isTalismanEnabled(player, talismanName))
                            return;
                        event.setAmount(event.getAmount() *  (1f + Config.SLOWED_TALISMAN_INCREASED_DAMAGE.get().floatValue()));
                    }
                }
            }
        }
