...
public class PocketMirrorItem extends TalismanItem implements ICurioItem, Pocket_Mirror_Provider {

    private static final Logger LOGGER = LogManager.getLogger(Fargos.MODID);
    public static final String charmName = "pocket_mirror";

    public PocketMirrorItem() {
    ...
    public static boolean checkConfigEnabledStatus() {
        ...
        try {
            ...
            LOGGER.error("Error checking config status for " + charmName); // Log the error
        }
        return isEnabled;
    }

    private static void negateNegativeEffects(Player player) {
        List<MobEffectInstance> targetedEffects = player.getActiveEffects().stream()
                .filter(effectInstance -> {
                    MobEffect effect = effectInstance.getEffect().value();
                    boolean isTargeted = effect.equals(MobEffects.BLINDNESS.value()) ||
                            effect.equals(MobEffects.CONFUSION.value());
                    return isTargeted;
                })
                .toList();
