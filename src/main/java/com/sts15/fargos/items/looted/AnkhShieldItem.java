...
public class AnkhShieldItem extends TalismanItem implements ICurioItem, Ankh_Shield_Provider {
    ...
    private static void negateNegativeEffects(Player player) {
        List<MobEffectInstance> targetedEffects = player.getActiveEffects().stream()
                .filter(effectInstance -> {
                    ...
                    return effect.equals(MobEffects.POISON.value()) ||
                            effect.equals(MobEffects.DARKNESS.value()) ||
                            effect.equals(MobEffects.MOVEMENT_SLOWDOWN.value());
                })
                .toList();
